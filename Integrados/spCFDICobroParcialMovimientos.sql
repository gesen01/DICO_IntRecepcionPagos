...
IF EXISTS(SELECT * FROM sysobjects WHERE TYPE='p' AND NAME='spCFDICobroParcialMovimientos')
DROP PROCEDURE spCFDICobroParcialMovimientos
GO
--Nota: Para esta integracion se sugiere usar el natural de version 6000 y revisar su funcioonamiento antes de ejecutar este script integrado
	...
	-----------------------------------------------------------------------------------------------		  
		...
		SELECT mf.OID,'CXC', mf.OMov,mf.OMovID, Clave ,mf.DID
		 FROM  MovTipo AS mt 
			...
			END	
	-----------------------------------------------------------------------------------------------
		--Se verifica que el cobro no haya sido timbrado desde el m�dulo de CXC
			...
			BEGIN
				...
				BEGIN 
					...
					--Si el movimiento tiene como origen una Factura en CXC, se considera para timbrar
						IF @Clave IN ('CXC.F', 'CXC.CA', 'CXC.FB','CXC.N') 
						BEGIN
			...
			--Si hay registros en la tabla CFDICobroVenta asociados al movimiento de Deposito, quiere decir que est� movimiento se puede timbrar 			
		...
		BEGIN 
			...
			-- Se actualiza la informacion de la cuenta del cliente de Informaci�n Adicional para que coincida el ingreso. BUG 16152													
