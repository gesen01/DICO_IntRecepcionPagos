...
BEGIN
	...
	DECLARE
	  ...
	  (
		...
		SELECT cd.ID, ROUND(ISNULL(CASE WHEN e.Clave <> 'MXN' AND @MonedaDocto = 'MXN' THEN ROUND(cd.Importe, @DecimalesDR, 0) * (c.TipoCambio)	
								...
								END, 0), @DecimalesDR, 0), mt.Clave, mtd.Clave, mt.SubClave, 
									...
									CASE WHEN @TipoCFD  = 'Ant' THEN ISNULL(mt.CFD ,0) ELSE ISNULL(mt.CFD, 0) END , ISNULL(mt.RecepcionPagosParcialidad, 1), c.FechaEmision, c.FechaOriginal, cd.Aplica, cd.AplicaID---LOSD ERROR EN MOVTIPO POR CAMPO CFDFLEX
		FROM CxcD AS cd
      ...
      SELECT @MontoPagado =  @MontoPagado*(@TipoCambioP/@TipoCambioDR)
		...
		INSERT MontosXMLFactura		(IDVenta,IDCobro,NumParcialidad, ImpPagado,   ImpSaldoAnt,	ImpSaldoInsoluto,	AplicaFactor, Estacion, Empresa, Factoraje )
		SELECT							@IDVenta,@cID	,@NumParcialidad,@MontoPagado,@SaldoAnterior,@ImpSaldoInsoluto,@AplicaFactor, @Estacion,@Empresa,@Factoraje
